...
contract ExpensiveJpeg is ERC721 {
    ...
    function approveAll(address spender) public returns (uint[] memory) {
        ...
        for (uint i; i < collection.length; i++) {
            _approve(spender, collection[i]);
        }
        return collection;
    }

    function _fetchInventory(address user)
        internal
        view
        returns (uint[] memory)
    {
    ...
    constructor(string memory uri) ERC721("Expensive JPEG", "$JPG") {
        baseUri = uri;
    }
}
